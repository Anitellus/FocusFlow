...
function renderAnalytics(days) {
    ...
    allTasks.forEach(t => {
        if (t.isCompleted && t.completionDate) {
            ...
            if (days === 9999 || cDate >= cutoff) {
                ...
                logsHTML += `<div class="px-6 py-3 flex justify-between items-center text-xs"><span class="font-semibold text-slate-700 flex items-center gap-2"><i data-lucide="check-circle" class="w-3.5 h-3.5 text-emerald-500"></i> ${t.title}<span class="text-[10px] text-slate-400 font-normal">(${t.projectTitle})</span></span><span class="text-slate-500 font-mono">${formatTimeCompact(t.actualTime)}</span></div>`;
            }
    ...
    if (totalTimeSec === 0 && logs.length > 0) totalTimeSec = logs.reduce((acc, l) => acc + (l.durationSeconds || 0), 0);
    ...
    document.getElementById('circadian-bars-grid').innerHTML = hours.map(m => `<div class="h-10 bg-slate-100 rounded-sm flex items-end overflow-hidden"><div class="w-full bg-amber-500 rounded-sm" style="height: ${Math.round((m / maxH) * 100)}%" title="${m} mins"></div></div>`).join('');

    // --- Resolved Object Permanence Guardian Bug ---
    ...
    const dormantProjects = allProjects
        ...
        .map(p => {
            ...
            const lastTaskUpdate = times.length > 0 ? Math.max(...times) : new Date(p.createdAt || 0).getTime();
            
            // Critical fix: Compare against actual session log activity, not just task completion
            const projLogs = (appState.sessionLogs || []).filter(l => l.projectId === p.id);
            const lastLogUpdate = projLogs.length > 0 ? Math.max(...projLogs.map(l => new Date(l.endedAt).getTime())) : 0;
            
            const lastActivity = Math.max(lastTaskUpdate, lastLogUpdate);
            const daysDormant = Math.floor((now.getTime() - lastActivity) / 86400000);
    ...
    if (dormantProjects.length === 0) {
        ...
        dormantList.innerHTML = dormantProjects.map(p => `<div class="flex items-center justify-between p-2 bg-slate-50 border border-slate-200 rounded-xl text-xs"><span class="font-bold text-slate-700">${p.title} (${p.daysDormant} days untouched)</span><button onclick="selectProject('${p.id}'); switchView('focus')" class="px-2.5 py-1 bg-brand-600 text-white font-bold rounded-lg text-[10px]">Reactivate</button></div>`).join('');
    }
